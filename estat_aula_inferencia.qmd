...
::: {.callout-tip title="Eficiência relativa entre estimadores não enviesados" }
A variância de um estimador é simplesmente $Var(T)=E(T^2)-E(T)^2$. 

Seja $T$ e $\tilde T$ estimadores não enviesados do parâmetro $\theta$. Dizemos que $T$ é relativamente mais eficiente do que $\tilde T$ se
$$\begin{align*}Var(T)<Var(\tilde T),\end{align*}$$
para todo $\theta\in\Theta$.
:::

::: {.callout-note title="Estimador não eficiente de $\mu$" collapse=true}

:::

Nós não podemos comparar diretamente a variância de estimadores enviesados. Nesse caso nós calculamos o *erro quadrático médio* (EQM), definido como variância mais viés ao quadrado.
...
::: {#exr-intervalo-confianca-media-1}

:::

::: {#exr-intervalo-confianca-media-2}

:::

::: {#exr-intervalo-confianca-variancia-1}

:::

::: {#exr-transformacao-monotonica-extremos}
